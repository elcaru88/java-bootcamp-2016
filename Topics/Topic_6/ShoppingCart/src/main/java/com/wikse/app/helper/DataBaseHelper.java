...
public class DataBaseHelper {
	...
	public static User updateUser(User userUpdate) {
		List<User> list=DataBaseHelper.users;
		for (int i = 0; i < list.size(); i++) {
			User u=list.get(i);
			if (u.getIdUser()==userUpdate.getIdUser()) {
				list.set(i, userUpdate);
				return userUpdate;
			}
		}
		User notUpdated= new User();
		notUpdated.setIdUser(0);
		return notUpdated;
	}

	

	public static void setListUsers(List<User> users) {
		DataBaseHelper.users=users;
	}
	...
	public static boolean checkUserByName(String nameUser) {
		for (User user : DataBaseHelper.users) {
